...
structure EliminateEq : ELIMINATE_EQ =
  struct
    ...
    fun monolet {lvar, Type, bind, scope} =
      LET {pat=[(lvar,[],Type)], owns=false, bind=bind, scope=scope}
         ...
         | PRIM(EQUALprim {instance}, [lexp1, lexp2]) =>
               (case instance
                  of RECORDtype ([tau,_],_) =>
                   ...
                   | _ => die "f.EQUALprim")
         | LET {pat=[(lvar,tvs,tau)],owns=owns,bind,scope} =>
            ...
            in
              LET {pat=[(lvar,tvs,tau')], owns=owns, bind=bind', scope=scope'}
            end
         | LET {pat=nil,owns,bind,scope} =>
            let val bind' = t env bind
              val scope' = t env scope
            in LET {pat=nil, owns=owns, bind=bind', scope=scope'}
            end
