...
fun generate_samples(values', 0) = 
  let oval values = values'
  in
    [change(values)]
  end
  | generate_samples(values', num) = 
  let oval values = values'
  in
    (change values)::generate_samples(values, num-1)
  end  
...
fun findMax(poly, len) = let
  ...
  fun findMax'(cur, 0) = copy cur
    | findMax'(cur', iterations) = let
      (* ensures that samples lifetime is not as long long cur which stays a lot *)
      oval cur = cur'
      val maximum_values = let
      ...
      val final_values = copy maximum_values
      val _ = resetRegions maximum_values
    in
      findMax'(final_values, iterations-1)
    end

  val values = findMax'(zeros len, GENERATIONS)
